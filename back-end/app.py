...
def homepage():
    ...
    food = db.session.query(transactions).filter_by(category='Food').all()
    incomes = transactions.query.filter_by(Type='Income').all()

    # Process the data to create the chart data
    expense_total = sum([float(t.amount) for t in expenses])
    income_total = sum([float(t.amount) for t in incomes])
    chart_data = [expense_total, income_total]


    return render_template("homepage.html",name=name, chart_data=chart_data)
